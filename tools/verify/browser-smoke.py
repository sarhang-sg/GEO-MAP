...
def main() -> int:
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            if page.locator('[data-map-mode-cycle="true"]').count() != 1:
                raise AssertionError("map mode must use exactly one animated cycle control")
            if page.locator("html").get_attribute("data-ui-mode") != "dark":
                raise AssertionError("fresh launches must start in dark/night mode")
            ...
            mobile_primary_controls = page.evaluate(
                """() => Object.fromEntries(['userAccountButton', 'locateButton', 'layersButton'].map(id => {
                  const element = document.getElementById(id);
                  return [id, element ? getComputedStyle(element).display : 'missing'];
                }))"""
            )
            # CSSOM serializes the modern two-keyword `inline flex` display
            # value as `flex` in current Chromium, while older engines can
            # still report `inline-flex`.  Both values describe the intended
            # visible flex controls; the secondary layers control must remain
            # collapsed.
            if (
                mobile_primary_controls.get("userAccountButton") not in {"flex", "inline-flex"}
                or mobile_primary_controls.get("locateButton") not in {"flex", "inline-flex"}
                or mobile_primary_controls.get("layersButton") != "none"
            ):
                raise AssertionError(f"mobile account/location controls are not canonical: {mobile_primary_controls}")

            # Exercise the single mode control until light mode is active, then
            # verify that icons use readable ink without outline filters and
            # that the search field cannot paint outside its glass capsule.
            for _ in range(3):
                if page.locator("html").get_attribute("data-ui-mode") == "light":
                    break
                page.locator("#mapModeCycleButton").evaluate("element => element.click()")
                page.wait_for_timeout(250)
            if page.locator("html").get_attribute("data-ui-mode") != "light":
                raise AssertionError("single mode control could not reach light mode")
            light_contract = page.evaluate(
                """() => {
                  const search = document.querySelector('#placeSearch');
                  const icon = document.querySelector('#locateButton .nav-ui-icon');
                  const threeD = document.querySelector('#threeDButton');
                  const eye = document.querySelector('#controlsVisibilityButton');
                  const foreground = selector => {
                    const element = document.querySelector(selector);
                    return element ? getComputedStyle(element).color : 'missing';
                  };
                  search?.focus();
                  const alpha = value => value === 'transparent' || value === 'rgba(0, 0, 0, 0)';
                  return {
                    inputBackground: search ? getComputedStyle(search).backgroundColor : 'missing',
                    iconFilter: icon ? getComputedStyle(icon).filter : 'missing',
                    threeDTransparent: threeD ? alpha(getComputedStyle(threeD).backgroundColor) : true,
                    eyeTransparent: eye ? alpha(getComputedStyle(eye).backgroundColor) : true,
                    shellBefore: getComputedStyle(document.querySelector('.map-shell'), '::before').display,
                    shellAfter: getComputedStyle(document.querySelector('.map-shell'), '::after').display,
                    brandHintColor: foreground('.brand-card__hint'),
                    searchIconColor: foreground('.search-card__icon'),
                    moreIconColor: foreground('#actionsToggleButton'),
                    feedbackIconColor: foreground('#feedbackQuickButton'),
                  };
                }"""
            )
            if light_contract["inputBackground"] not in ("rgba(0, 0, 0, 0)", "transparent"):
                raise AssertionError(f"light search input paints an overflow surface: {light_contract}")
            if "drop-shadow" in light_contract["iconFilter"]:
                raise AssertionError(f"light icons still use outline filters: {light_contract}")
            if light_contract["threeDTransparent"] or light_contract["eyeTransparent"]:
                raise AssertionError(f"light map controls are missing glass surfaces: {light_contract}")
            if light_contract["shellBefore"] != "none" or light_contract["shellAfter"] != "none":
                raise AssertionError(f"light shell decoration can protrude through search: {light_contract}")
            unreadable_light_ink = {
                key: value
                for key, value in light_contract.items()
                if key.endswith("Color") and value not in {"rgb(51, 65, 85)", "rgb(30, 41, 59)"}
            }
            ...
            try:
                ...
                offline_asset_statuses = page.evaluate(
                    """async () => Promise.all(
                      ['/offline.css', '/offline.js'].map(async path =>
                        (await fetch(path, { cache: 'reload' })).status)
                    )"""
                )
                if offline_asset_statuses != [200, 200]:
                    raise AssertionError(
                        "offline controls were not available from the controlled shell: "
                        f"{offline_asset_statuses}"
                    )
                page.goto(f"{BASE_URL}/offline.html", wait_until="domcontentloaded", timeout=15_000)
                if page.locator("[data-offline-action]").count() != 2:
                    raise AssertionError("offline fallback is missing retry/back controls")
                if page.locator(".offline-actions a").count() != 0:
                    raise AssertionError("offline fallback still exposes a selectable raw link")
                page.goto(BASE_URL, wait_until="domcontentloaded", timeout=15_000)
            ...
            tutorial_context = browser.new_context(viewport={"width": 390, "height": 844}, locale="ku")
            tutorial_page = tutorial_context.new_page()
            tutorial_page.goto(BASE_URL, wait_until="domcontentloaded", timeout=30_000)
            tutorial_page.locator(".nav-tutorial.is-positioned").wait_for(state="visible", timeout=45_000)
            tutorial_page.locator('[data-tutorial-action="minimize"]').click()
            tutorial_page.locator(".nav-tutorial.is-minimized").wait_for(state="attached", timeout=10_000)
            tutorial_page.locator('[data-tutorial-action="restore"]').click()
