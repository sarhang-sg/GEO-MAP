...
const navigationHistoryStore = await readText("src/lib/navigation-history-store.ts");
const navigationHistoryMigration = await readText("supabase/migrations/20260908_000024_navigation_history_cloud_sync.sql");
...
const gpsLayers = await readText("src/lib/live-location-layers.ts");
const offlinePack = await readText("src/lib/offline-map-pack.ts");
const hardware = await readText("src/lib/hardware-profile.ts");
const viewportPoi = await readText("src/lib/viewport-poi-source.ts");
const professionalTheme = await readText("src/styles/nav-kurd-professional.css");
...
assert(manifest.background_color === "#07111F" && manifest.theme_color === "#07111F", "PWA first paint is not dark/night.");
...
assert(
  userContribution.includes("restoreEditorAfterPhotoPicker")
    && userContribution.includes("armPhotoPickerFallbackRestore")
    && userContribution.includes('this.photoPickerScroll = null')
    && !userContribution.includes('content.dataset.nativePickerRestored')
    && userContribution.includes("data-user-photo-preview")
  ...
  "Mobile gallery return can still replace or collapse the contribution editor."
);
...
assert((shell.match(/data-map-mode="/gu) ?? []).length === 1 && shell.includes('data-map-mode-cycle="true"'), "Map mode is not one animated cycle control.");
assert(shell.includes('data-map-mode="night"') && indexHtml.includes('data-ui-mode="dark"'), "Fresh app markup does not start in dark/night mode.");
assert(!routing.includes("data-route-clear") && routing.includes("createNavigationHud") && routing.includes("routeCancel"), "Start/Cancel navigation flow or trash removal is incomplete.");
assert(routing.includes('recordNavigationHistory("arrived")') && routing.includes('recordNavigationHistory("cancelled")') && routing.includes("persistNavigationHistory(entry)") && navigationHistoryStore.includes("queueNavigationHistory(entry, userId)") && navigationHistoryStore.includes("loadSynchronizedNavigationHistory(userId)"), "Arrival and cancellation are not passed to the durable navigation-history writer.");
assert(navigationHistoryStore.includes('const STORAGE_KEY = "nav-kurd-navigation-history-v1"') && navigationHistoryStore.includes("syncAtlasNavigationHistory") && navigationHistoryStore.includes("loadAtlasNavigationHistory"), "Navigation-history offline queue and cloud reconciliation are incomplete.");
assert(atlasPlaces.includes('.from("atlas_navigation_history")') && navigationHistoryMigration.includes("enable row level security") && navigationHistoryMigration.includes("to authenticated") && navigationHistoryMigration.includes("grant select, insert, update, delete"), "Navigation history is not protected and synchronized through the account database.");
assert(main.includes("MAP_CONTROLS_IDLE_DELAY_MS = 10_000") && main.includes('classList.toggle("map-controls-idle"'), "Navigation-friendly map control auto-hide is missing.");
assert(main.includes('.nav-tutorial:not([hidden]):not(.is-minimized)') && tutorial.includes('data-tutorial-action="minimize"') && tutorial.includes('data-tutorial-action="restore"') && tutorial.includes("adaptiveAccount") && tutorial.includes('#brandAboutButton'), "Tutorial minimize/restore or adaptive account/About targeting is incomplete.");
assert(shell.includes('navIcon("a1")') && shell.includes('assets/icons/nav-kurd/a2.svg'), "Administrator and satellite SVG controls are not installed.");
assert(main.includes("renderAccountPresentation") && main.includes("ownerStudioButton.hidden = !isAdmin") && main.includes("userAccountButton.hidden = Boolean(isAdmin)") && main.includes("user-account-button__avatar"), "Role-aware account control or authenticated avatar presentation is incomplete.");
assert(!gpsLayers.includes("location-pulse") && !gpsLayers.includes("location-dot-halo") && !gpsLayers.includes("location-direction-halo"), "Obsolete GPS shadow/pulse layers remain.");
assert(main.includes("onNavigationStateChange") && professionalTheme.includes(".route-navigation-hud") && professionalTheme.includes(".map-shell.is-navigating.map-controls-idle .map-sheet"), "Navigation HUD or distraction-free navigation state is incomplete.");
assert(professionalTheme.includes('html[data-ui-mode="light"][data-nav-kurd-theme="professional"]') && professionalTheme.includes(".support-section.support-section--neon") && professionalTheme.includes(".nav-tutorial__matched-icon") && professionalTheme.includes("filter: brightness(0) !important") && professionalTheme.includes("backdrop-filter: blur(14px)"), "Unified light/help/tutorial visual overrides are incomplete.");
assert(viewportPoi.includes("leafLoads") && viewportPoi.includes("force-cache") === false && viewportPoi.includes("signature = leaves.map"), "Viewport POI request coalescing or stable zoom identity is missing.");
assert((viewportPoi.match(/lifetimeAborter\.abort\(\)/gu) ?? []).length === 1 && /destroy:\s*\(\)\s*=>\s*\{[\s\S]*lifetimeAborter\.abort\(\)/u.test(viewportPoi), "POI lifetime requests can be aborted by a temporary hide/pan state.");
