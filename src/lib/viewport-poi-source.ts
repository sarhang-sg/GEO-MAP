...
export function installViewportPoiSourceController(options: ViewportPoiSourceOptions): ViewportPoiSourceController {
  ...
  const leafLoads = new Map<string, Promise<ViewportPoiShard>>();
  const failedUntil = new Map<string, number>();
  const lifetimeAborter = new AbortController();
  ...
  const loadLeaf = (leaf: ViewportPoiLeaf): Promise<ViewportPoiShard> => {
    const cached = shardCache.get(leaf.key);
    if (cached) {
      touchCache(leaf.key, cached);
      return Promise.resolve(cached);
    }
    const inFlight = leafLoads.get(leaf.key);
    ...
    const task = fetchPersistentJson<ViewportPoiShard>(
      ...
      .catch((error) => {
        if (isAbortError(error) || lifetimeAborter.signal.aborted) throw error;
        failedUntil.set(leaf.key, Date.now() + retryCooldownMs);
        throw error;
      })
      .finally(() => { leafLoads.delete(leaf.key); });
    leafLoads.set(leaf.key, task);
    return task;
  };
  ...
  const loadBounded = async (
    ...
    await Promise.all(Array.from({ length: Math.min(concurrentLoads, leaves.length) }, async () => {
      while (cursor < leaves.length) {
        ...
        try {
          // Once a shard request has started, let the immutable response finish
          // into the persistent/memory cache. A quick pan only stops scheduling
          // additional off-screen leaves; it never throws away downloaded bytes.
          const entry = { leaf, shard: await loadLeaf(leaf) };
          results[index] = entry;
        } catch {
