...
export function ensureLiveLocationLayers(
  ...
  if (!map.getLayer("location-pulse-outer")) {
    map.addLayer({
      id: "location-pulse-outer",
      type: "circle",
  ...
  if (!map.getLayer("location-pulse-inner")) {
    map.addLayer({
      id: "location-pulse-inner",
      type: "circle",
      source: LOCATION_POINT_SOURCE,
      paint: {
        "circle-radius": 10,
  ...
  if (!map.getLayer("location-dot-halo")) {
    map.addLayer({
      id: "location-dot-halo",
      type: "circle",
      source: LOCATION_POINT_SOURCE,
      paint: {
        "circle-radius": ["interpolate", ["linear"], ["zoom"], 6, 7.5, 14, 12.8, 18, 15.5],
        "circle-color": "#7b68ff",
        "circle-opacity": 0.25,
        "circle-blur": 0.36,
        "circle-pitch-alignment": "map",
  ...
  if (!map.getLayer("location-direction-halo")) {
    map.addLayer({
      id: "location-direction-halo",
      type: "circle",
      source: LOCATION_POINT_SOURCE,
      filter: ["==", ["get", "hasHeading"], true],
      paint: {
        "circle-radius": ["interpolate", ["linear"], ["zoom"], 6, 7.6, 14, 12.5, 18, 14.9],
