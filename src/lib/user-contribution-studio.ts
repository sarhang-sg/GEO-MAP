...
type UserStudioOptions = {
  ...
  onAdminRoleChange?: (isAdmin: boolean | null) => void;
...
function userIcon(name: "close" | "plus" | "back" | "chevron" | "pin" | "image" | "trash" | "readAll" | "bell" | "signout" | "places" | "messages" | "account"): string {
  const paths: Record<string, string> = {
    ...
    places: '<path d="M4 7.5 12 3l8 4.5v9L12 21l-8-4.5v-9Z"/><path d="M12 12v9M4.5 7.5 12 12l7.5-4.5"/>',
    messages: '<path d="M5 5h14a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H10l-5 4v-4a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2Z"/><path d="M7 9h10M7 13h7"/>',
...
export class UserContributionStudio {
  ...
  constructor(options: UserStudioOptions) {
    ...
    document.addEventListener("visibilitychange", () => { if (document.visibilityState === "hidden") this.captureEditorDraft(); });
  }
  ...
  async syncRole(): Promise<AtlasAuthIdentity | null> {
    try {
      ...
      this.options.onAdminRoleChange?.(identity?.role === "admin");
      return identity;
    } catch {
      this.options.onAdminRoleChange?.(null);
      return null;
    }
  }
  ...
  private async refresh(): Promise<void> {
    ...
    try {
      ...
      this.options.onAdminRoleChange?.(this.identity?.role === "admin");
      if (!this.identity) {
        this.profile = null;
        this.places = [];
        this.notifications = [];
    ...
    this.host.innerHTML = `
      ...
      <section class="user-contrib__panel" role="dialog" aria-modal="true" aria-labelledby="userContribTitle" dir="${languageDirection(language)}">
        <header class="user-contrib__header">
          ...
          <button type="button" data-user-action="close" aria-label="${escapeText(copy.close)}">${userIcon("close")}</button>
        </header>
    ...
    return `<div class="user-confirm" role="presentation">
      ...
      <section class="user-confirm__panel" role="alertdialog" aria-modal="true" aria-labelledby="userConfirmTitle" aria-describedby="userConfirmMessage" dir="${languageDirection(language)}">
        ...
        <div class="user-confirm__actions">
          ...
          <button type="button" class="user-confirm__delete ${isSignOut ? "is-signout" : ""}" data-user-action="confirm-proceed" ${(this.busy || (isAccount && !this.accountDeleteAcknowledged)) ? "disabled" : ""}>${userIcon(isSignOut ? "signout" : "trash")}<span>${escapeText(isSignOut ? copy.signOutConfirmAction : copy.confirmDelete)}</span></button>
        </div>
      </section>
    </div>`;
  }
  ...
  private renderDashboard(copy: Copy, language: StudioLanguage): string {
    ...
    const avatarInitial = displayName.trim().slice(0, 1).toUpperCase() || "N";
    const avatarUrl = safeAvatarUrl(this.profile?.avatar_url ?? this.identity?.avatarUrl);
    const avatar = `<span class="user-account-card__avatar-fallback" aria-hidden="true">${escapeText(avatarInitial)}</span>${avatarUrl ? `<img src="${escapeText(avatarUrl)}" alt="" referrerpolicy="no-referrer" loading="eager" decoding="async" data-user-avatar>` : ""}`;
  ...
  private attachEvents(): void {
    ...
    form?.querySelector<HTMLInputElement>("[data-user-photo-input]")?.addEventListener("change", (event) => {
