...
function ownerUtilityIcon(name: "readAll" | "trash" | "chevron" | "signout" | "pin"): string {
  if (name === "trash") {
    return `<img class="owner-ui-icon owner-ui-icon--asset" src="${appUrl("assets/icons/nav-kurd/delete.svg")}" alt="" aria-hidden="true" draggable="false">`;
  }
...
export class OwnerStudio {
  ...
  private async refresh(): Promise<void> {
    ...
    try {
      ...
      this.setMessage("");
      [this.places, this.notifications, this.feedback, this.navigationHistory] = await Promise.all([
        loadManagedAtlasPlaces(),
        loadAtlasNotifications(),
        loadManagedAtlasFeedback(),
        loadSynchronizedNavigationHistory(this.identity.userId)
      ]);
      ...
      <section class="owner-studio__panel" role="dialog" aria-modal="true" aria-labelledby="ownerStudioTitle" dir="${languageDirection(language)}">
        <div class="owner-studio__scroll">
          <header class="owner-studio__header">
            ...
            <button class="dialog-close-button owner-studio__icon-button" type="button" data-owner-action="close" aria-label="${escapeText(copy.close)}">${dialogCloseIcon()}</button>
          </header>
  ...
  private renderChoiceDialog(copy: StudioCopy, language: StudioLanguage): string {
    ...
    return `<div class="owner-choice" role="presentation">
      ...
      <section class="owner-choice__panel" role="dialog" aria-modal="true" aria-labelledby="ownerChoiceTitle" dir="${languageDirection(language)}">
        <header class="owner-choice__header"><div><p>NAV KURD</p><h3 id="ownerChoiceTitle">${escapeText(title)}</h3></div><button class="dialog-close-button" type="button" data-owner-action="choice-close" aria-label="${escapeText(copy.choiceClose)}">${dialogCloseIcon()}</button></header>
  ...
  private renderConfirmation(copy: StudioCopy, language: StudioLanguage): string {
    ...
    const isHistory = pending.kind === "history" || pending.kind === "history-all";
    ...
    const title = isSignOut ? copy.signOutConfirmTitle : isHistory ? historyCopy.title : copy.confirmTitle;
    const detail = isSignOut ? copy.signOutConfirmBody
      ...
      : pending.kind === "history" ? historyCopy.deleteConfirm
      : pending.kind === "history-all" ? historyCopy.clearConfirm
      : notificationDelete;
  ...
  private renderList(copy: StudioCopy, language: StudioLanguage): string {
    ...
    const renderPlaceRows = (selectedPlaces: AtlasPlace[], reviewMode: boolean, emptyMessage: string): string => selectedPlaces.length
      ? selectedPlaces.map((place) => {
        ...
        const actions = isReview
          ...
          : `<button type="button" data-owner-action="edit" data-id="${escapeText(place.id)}">${escapeText(copy.edit)}</button><button type="button" class="owner-place-row__delete" data-owner-action="delete" data-id="${escapeText(place.id)}" aria-label="${escapeText(copy.delete)}" title="${escapeText(copy.delete)}">${ownerUtilityIcon("trash")}</button>`;
        ...
        const contributorAvatar = contributor?.avatar_url
          ...
          : `<img src="${appUrl("assets/icons/nav-kurd/profile.svg")}" alt="" aria-hidden="true" loading="lazy">`;
    ...
    const feedbackRows = this.feedback.slice(0, 100).map((item) => {
      ...
      return `<article class="owner-feedback" data-status="${escapeText(item.status)}">
        ...
        <div class="owner-feedback__actions">
          ...
          <button type="button" class="is-danger" data-owner-action="feedback-delete" data-id="${escapeText(item.id)}">${ownerUtilityIcon("trash")}<span>${escapeText(deleteMessageLabel)}</span></button>
        </div>
      </article>`;
    ...
    const historyRows = this.navigationHistory.slice(0, 40).map((entry) => {
      const coveredMeters = entry.status === "arrived" ? entry.planned_distance_meters : Math.max(0, entry.planned_distance_meters - entry.remaining_distance_meters);
      const distance = coveredMeters < 1000 ? `${Math.round(coveredMeters).toLocaleString(historyLocale)} m` : `${(coveredMeters / 1000).toLocaleString(historyLocale, { maximumFractionDigits: 1 })} km`;
      const date = new Intl.DateTimeFormat(historyLocale, { dateStyle: "medium", timeStyle: "short" }).format(new Date(entry.ended_at));
      return `<article class="owner-navigation-history__item" data-status="${entry.status}"><span class="owner-navigation-history__pin" aria-hidden="true">${ownerUtilityIcon("pin")}</span><div><strong>${escapeText(entry.destination)}</strong><small>${escapeText(date)} · ${escapeText(distance)}</small></div><b>${escapeText(entry.status === "arrived" ? historyCopy.arrived : historyCopy.cancelled)}</b><button type="button" data-owner-action="history-delete" data-id="${escapeText(entry.id)}" aria-label="${escapeText(historyCopy.delete)}" title="${escapeText(historyCopy.delete)}">${ownerUtilityIcon("trash")}</button></article>`;
    }).join("") || `<p class="owner-studio__empty">${escapeText(historyCopy.empty)}</p>`;
    const historyPanel = `<section class="owner-navigation-history"><header><h3>${escapeText(historyCopy.title)} <span>${this.navigationHistory.length}</span></h3>${this.navigationHistory.length ? `<button type="button" data-owner-action="history-clear">${ownerUtilityIcon("trash")}<span>${escapeText(historyCopy.clear)}</span></button>` : ""}</header><div>${historyRows}</div></section>`;
  ...
  private async handleAction(action: string, id?: string): Promise<void> {
    ...
    if (action === "history-delete" && id) {
      this.pendingConfirmation = { kind: "history", historyId: id };
      this.render();
      return;
    }
    if (action === "history-clear" && this.navigationHistory.length > 0) {
      this.pendingConfirmation = { kind: "history-all" };
      this.render();
      return;
    }
    ...
    if (action === "confirm-proceed") {
      ...
      if (pending.kind === "history" || pending.kind === "history-all") {
        await this.runTask(async () => {
          const historyCopy = ownerNavigationHistoryCopy(this.options.getLanguage());
