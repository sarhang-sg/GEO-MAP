...
function feedbackIcon(name: "copy" | "mail" | "send" | "device"): string {
    ...
    this.host.innerHTML = `<div class="feedback-studio__backdrop" data-feedback-action="close"></div>
      <section class="feedback-studio__panel" role="dialog" aria-modal="true" aria-labelledby="feedbackTitle" dir="${languageDirection(language)}">
        <div class="feedback-studio__scroll">
          <header><div><p>NAV KURD</p><h2 id="feedbackTitle">${escapeText(copy.title)}</h2><span>${escapeText(copy.subtitle)}</span></div><button class="dialog-close-button feedback-studio__close" type="button" data-feedback-action="close" aria-label="${escapeText(copy.close)}">${dialogCloseIcon()}</button></header>
