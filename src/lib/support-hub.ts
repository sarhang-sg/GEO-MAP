...
function personAvatar(_name: string, avatarUrl: string | null): string {
  if (avatarUrl) return `<img src="${escapeHtml(avatarUrl)}" alt="" referrerpolicy="no-referrer" loading="lazy">`;
  return `<img src="${appUrl("assets/icons/nav-kurd/profile.svg")}" alt="" aria-hidden="true" loading="lazy">`;
}
