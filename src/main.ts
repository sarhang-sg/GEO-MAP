...
function applyUiMode(mode: MapMode): void {
  const uiMode = mode === "street" ? "light" : mode === "night" ? "dark" : "satellite";
  document.documentElement.dataset.uiMode = uiMode;
  document.documentElement.style.colorScheme = uiMode === "light" ? "light" : "dark";
  const theme = document.querySelector<HTMLMetaElement>('meta[name="theme-color"]');
  if (theme) theme.content = uiMode === "light" ? "#EDF2F7" : uiMode === "satellite" ? "#07111FCC" : "#07111F";
}
applyUiMode(recoveredLifecycleState?.mapMode ?? "night");
...
mapShell.dataset.mapMode = recoveredLifecycleState?.mapMode ?? "night";
...
function mapModeLabel(mode: MapMode, language = currentLanguage()): string {
  const copy = UI[language];
  return mode === "street" ? copy.streetStyle : mode === "night" ? copy.nightStyle : copy.satelliteStyle;
}

function syncMapModeControl(mode: MapMode): void {
  mapStyleButtons.forEach((button) => {
    if (button.dataset.mapModeCycle === "true") button.dataset.mapMode = mode;
    const active = button.dataset.mapModeCycle === "true" || button.dataset.mapMode === mode;
    button.classList.toggle("is-active", active);
    button.setAttribute("aria-pressed", String(active));
    const label = mapModeLabel(mode);
    button.querySelector<HTMLElement>("[data-map-mode-label]")?.replaceChildren(document.createTextNode(label));
...
class KurdistanAtlasController {
  ...
  private mapMode: MapMode = recoveredLifecycleState?.mapMode === "satellite"
    ...
    : recoveredLifecycleState?.mapMode === "street" ? "street" : "night";
  ...
  private activePlacePopup: maplibregl.Popup | null = null;

  constructor() {
    applyUiMode(this.mapMode);
    mapShell.dataset.mapMode = this.mapMode;
    mapElement.dataset.mapMode = this.mapMode;
    syncMapModeControl(this.mapMode);
  ...
  private canonicalSatelliteDetailOpacity(): PropertyValueSpecification<number> {
...
function trustedAccountAvatar(value: string | null | undefined): string | null {
...
function renderAccountPresentation(identity: AtlasAuthIdentity | null): void {
  ...
  ownerStudioButton.dataset.authenticated = String(Boolean(isAdmin));
  const fallbackIcon = userAccountButton.querySelector<HTMLImageElement>(".nav-ui-icon");
  const existingAvatar = userAccountButton.querySelector<HTMLImageElement>(".user-account-button__avatar");
  const avatarUrl = isAdmin ? null : trustedAccountAvatar(identity?.avatarUrl);
  if (avatarUrl) {
    const avatar = existingAvatar ?? document.createElement("img");
    avatar.className = "user-account-button__avatar";
    avatar.alt = "";
    avatar.draggable = false;
    avatar.referrerPolicy = "no-referrer";
    avatar.decoding = "async";
    if (avatar.src !== avatarUrl) avatar.src = avatarUrl;
    if (!existingAvatar) userAccountButton.insertBefore(avatar, userAccountDot ?? null);
...
const tutorialController = installTutorialController({
  ...
  openAccount: async () => {
    ...
    await (await loadUserContributionStudio()).open();
  },
  closeAccount: () => userContributionStudio?.close(),
...
const MAP_CONTROLS_IDLE_DELAY_MS = 10_000;
let mapControlsIdleTimer: number | null = null;
...
function setMapControlsIdle(hidden: boolean): void {
  const shouldHide = hidden
    && !mapShell.classList.contains("map-controls-hidden")
    && !mapControlsAutoHideSuspended();
  mapShell.classList.toggle("map-controls-idle", shouldHide);
}

function revealMapControls(): void {
  setMapControlsIdle(false);
}
...
function syncControlsVisibilityButtonCopy(): void {
  ...
  controlsVisibilityButton.classList.toggle("is-active", controlsHidden);
}
...
mapStyleButtons.forEach((button) => button.addEventListener("click", () => {
  const current = button.dataset.mapMode as MapMode;
  const sequence: MapMode[] = satelliteEnabled ? ["street", "night", "satellite"] : ["street", "night"];
  const next = button.dataset.mapModeCycle === "true"
    ? sequence[(Math.max(0, sequence.indexOf(current)) + 1) % sequence.length]
    : current;
  void controller.setMapMode(next);
}));
...
function applySatelliteAvailability(): void {
  mapModeCycleButton.dataset.satelliteAvailable = String(satelliteEnabled);
  mapModeCycleButton.classList.toggle("is-satellite-unavailable", !satelliteEnabled);
  syncMapModeControl(mapModeCycleButton.dataset.mapMode as MapMode);
}
