...
Release ID: `2026-08-30-nav-kurd-v9.0.0`  
Release date: `2026-08-30`

NAV KURD 9 is a coordinated Web and Android visual, reliability and native-integration release. The browser application and the Flutter Android shell keep one shared map, search, account, offline and contribution experience while Android adds native lifecycle, downloads, sharing, notifications and a launcher widget.
...
- Added the supplied NAV KURD 9 cover to About and refreshed README cover/screenshots.
...
- The Flutter shell opens the same canonical NAV KURD 9 Web runtime and exposes only trusted-origin native handlers.
- Widget taps dispatch a warm-app event instead of reloading the WebView.
- Image-picker results use a protected Android `content://` URI through `FileProvider`.
- Benign cancelled subresource requests no longer pollute native diagnostics as `net::ERR_FAILED`.
- Version identity is unified at `9.0.0` / `90000`; service-worker cache schema is `90` and offline pack identity is `2027.12`.
...
The direct APK button stays hidden until a real signed `NAV-KURD-9.0.0.apk` passes size, MIME, ZIP structure, certificate workflow and SHA-256 metadata gates.
